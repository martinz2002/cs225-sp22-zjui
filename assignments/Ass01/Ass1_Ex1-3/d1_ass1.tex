...
\begin{questions}
	...
	\begin{parts}
		...
		\begin{solution}
			...
			% \begin{algorithm}
			% 	\SetKwData{Len}{len}\SetKwData{List}{list}
			...
			% 	\caption{Get List Length \texttt{get_lenth{my_list[]}}}\label{algoGetLen}
			% \end{algorithm}

% 			\begin{lstlisting}
% int countlength(TList l, int index)
% {
% 	// END_OF_LIST is a pre-defined symbol for (length+1)-th element
% 	if (l[0] == END_OF_LIST)
% 		return 0;

% 	// lastElements(int i) is a member function of class List, returning the sublist containing all elements from the i-th to the end of l
% 	if (l.getItem[index] == END_OF_LIST)
% 		return 1;
% 	else
% 		return (countlength(l.lastElements(index + 1));
% }
% 			\end{lstlisting}
