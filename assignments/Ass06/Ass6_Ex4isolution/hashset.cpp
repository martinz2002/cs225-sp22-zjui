...
template<class T> void hashset<T>::add(T item)
{
    ...
    index1 = hashfunction(item) % maxsize; // First determine the position index in the hash table, where the new value is stored, if free.
    index = index1;
    T value, victim;
    bool flag = false;
    ...
    {
        if (reprarray[index] != pt_nil)
        {
            value = *reprarray[index]; // get the competing value
            index2 = hashfunction(value) % maxsize; // Determine the position index in the hash table, where the competing value should have been stored
            if (value == item) // no competing value, but the item was found
            {
                if (location < 0)
                    return; // item found; no insertion
                else
                {
                    reprarray[index] = pt_nil; // store a placeholder, as a better position for the item has already been found
                    break; // leave the while loop and continue with the insertion of item in the better position
                }
            }
            if (((index - index1) % maxsize) < ((index - index2) % maxsize))
                /* item could be stored here replacing value, which has to be stored somewhere else */
            {
                location = index;
                victim = value;
                flag = true;
    ...
    // otherwise, if location >= 0 holds, we found a placeholder or a victim value that will be replaced, so the item will be stored at the location of this placeholder or victim
    if (location < 0)
        location = index;
    T *pt = new T;
    *pt = item;
    ...
    if (flag == true)
        add(victim);  // store the victim value that was replaced in a different location of the hashtable
    ++ numitems;
