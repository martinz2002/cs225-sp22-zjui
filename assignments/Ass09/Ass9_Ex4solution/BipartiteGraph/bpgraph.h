...
public:
    ...
    deepsearchtree<T> * dfsall(void);
private:
    void dfsnew (T startnode, deepsearchtree<T> *output);
    void _dfs(bool &boy, T node1, T node2, vertexlist<T> * vertices1, vertexlist<T> * vertices2, deepsearchtree<T> * output, markerlist<T> * markedvertices, int &dfspos, int &finishingtime);
        // first two arguments not used
    void _traverse_treeedge(bool &boy, T first, T second, deepsearchtree<T> * output, int &dfspos);
        // The first and third arguments will not be used.
    void _traverse_nontreeedge(bool &boy, T first, T second);
        // This function will do nothing.
    void _backtrack(bool & boy, T node1, T node2, deepsearchtree<T> * output, int &dfspos, int &finishingtime);
        // first two arguments not used
};
