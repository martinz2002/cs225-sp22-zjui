...
template<class T> void bpgraph<T>::addedge(T origin, T destination)
{
    if (!(*vertices1).contains(destination) && !(*vertices2).contains(origin))
    {
        (*vertices1).addedge(origin, destination);
        (*vertices2).addedge(destination, origin);
    }
    return;
}
...
template<class T> searchtree<T> * bpgraphtraversal<T>::bfs(T startnode)
{
    vertexlist<T> * vertices1 = bpgraph<T>::vertices1;
    vertexlist<T> * vertices2 = bpgraph<T>::vertices2;
    bool boy = (*vertices1).contains(startnode);
    if (!boy && !(*vertices2).contains(startnode))
    {
    ...
    do
    {
        ...
        while (!(*currentlayer).isempty())
        {
            ...
            elist<T> * neighbours;
            if (boy)
                neighbours = (*vertices1).edges(currentnode);
            else
                neighbours = (*vertices2).edges(currentnode);
            if (neighbours != 0)
            {
...
template<class T> deepsearchtree<T> * bpgraphtraversal<T>::dfs(T startnode)
{
    deepsearchtree<T> * output = new deepsearchtree<T>();
    dfsnew(startnode, output);
    return output;
}

template<class T> void bpgraphtraversal<T>::dfsnew(T startnode, deepsearchtree<T> * output)
{
    vertexlist<T> * vertices1 = bpgraph<T>::vertices1;
    vertexlist<T> * vertices2 = bpgraph<T>::vertices2;
    bool boy = (*vertices1).contains(startnode);
    ...
    _dfs(boy, startnode, startnode, vertices1, vertices2, output, markedvertices, dfspos, finishingtime);
    return;
}

/* auxiliary functions for depth-first search */
template<class T> void bpgraphtraversal<T>::_dfs(bool &boy, T node1, T node2, vertexlist<T> * vertices1, vertexlist<T> * vertices2, deepsearchtree<T> * output, markerlist<T> * markedvertices, int &dfspos, int &finishingtime)
{
    elist<T> * neighbours;
    if (boy)
    ...
    if (neighbours != 0)
    {
        ...
        for (int i = 0; i < (*neighbours).getlength(); i++)
        {
            ...
            if ((*markedvertices).ismarked(nextnode) == true)
                _traverse_nontreeedge(boy, node2, nextnode);
            else
            {
                _traverse_treeedge(boy, node2, nextnode, output, dfspos);
                (*markedvertices).mark(nextnode);
                boy = !boy;
                _dfs(boy, node2, nextnode, vertices1, vertices2, output, markedvertices, dfspos, finishingtime);
            }
            cursor = (*cursor).getnext();
        }
    }
    boy = !boy;
    _backtrack(boy, node1, node2, output, dfspos, finishingtime);
    return;
}

template<class T> void bpgraphtraversal<T>::_traverse_treeedge(bool &boy, T first, T second, deepsearchtree<T> * output, int &dfspos)
{
...
template<class T> void bpgraphtraversal<T>::_traverse_nontreeedge(bool &boy, T first, T second)
{
    return;
}

template<class T> void bpgraphtraversal<T>::_backtrack(bool &boy, T node1, T node2, deepsearchtree<T> * output, int &dfspos, int &finishingtime)
{
...
template<class T> deepsearchtree<T> *bpgraphtraversal<T>::dfsall(void)
{
    vertexlist<T> * vertices1 = bpgraph<T>::vertices1;
    vertexlist<T> * vertices2 = bpgraph<T>::vertices2;
    AList<T> *startlist = (*vertices1).getlist();
    (*startlist).concat(*((*vertices2).getlist()));
    deepsearchtree<T> * output = new deepsearchtree<T>();
    while ((*startlist).getlength() > 0)
    {
        T startnode = (*startlist)[1];
        (*startlist).remove(1);
        if (!(*output).occurs(startnode))
            dfsnew(startnode, output);
    }
    return output;
}
