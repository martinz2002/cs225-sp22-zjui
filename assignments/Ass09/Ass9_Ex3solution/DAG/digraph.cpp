...
template<class T> void AList<T>::takeoff(T item)
{
    if ((numitems == maxsize / 4) && (maxsize > minsize))
        deallocate();
    for (int k = 0; k < numitems; k++)
    {
        if (reprarray[k] == item)
        {
            for (int l = k; l < numitems - 1; l++)
            {
                reprarray[l] = reprarray[l+1];
            }
            --numitems;
            break;
        }
...
/* For acyclicity checking: get the vertices without incoming edges. */
template<class T> AList<T> *vertexlist<T>::candidatelist(void)
{
    vertex<T> *ptv = (*dummy).getnext();
    AList<T> *candidates = new AList<T>(); // initialise an empty list
    T item;
    /* First create the list of all vertices. */
    for (int i = 0; i < numvertices; i++)
    {
        item = (*ptv).getitem();
    ...
    /* Then eliminate those vertices that have an incoming edge. */
    elist<T> * incoming;
    ptv = (*dummy).getnext();
    /* Check the outgoing edges for all vertices. */
    for (int i = 0; i < numvertices; i++)
    {
        incoming = (*ptv).getedges(); // Get the vertices reachable by an edge
        /* If there is no edgelist, nothing needs to be done. Otherwise scan the list of reachable vertices. */
        if (incoming != 0)
        {
            neighbour<T> * neighb = (*incoming).getfirst();
            // scan the list of these vertices
            for (int j = 0; j < (*incoming).getlength(); j++)
            {
                item = (*neighb).getitem();
                if ((*candidates).member(item))  // item has an incoming edge
                    (*candidates).takeoff(item); // discard item from the candidate list
...
/* A digraph object just contains a pointer to a vertexlist. */
template<class T> digraph<T>::digraph(void)
{
    vertices = new vertexlist<T>;
}

template<class T> int digraph<T>::numvertices(void)
{
    return (*vertices).getlength();
}

template<class T> int digraph<T>::numedges(void)
{
    return (*vertices).countedges();
}

template<class T> void digraph<T>::addedge(T origin, T destination)
{
    (*vertices).addedge(origin, destination);
    return;
}

template<class T> void digraph<T>::addvertex(T item)
{
    (*vertices).addvertex(item);
    return;
}

template<class T> void digraph<T>::deleteedge(T origin, T destination)
{
    (*vertices).removeedge(origin, destination);
    return;
}

template<class T> void digraph<T>::deletevertex(T item)
{
    (*vertices).remove(item);
    return;
}

template<class T> edgelist<T> * digraph<T>::outgoingedges(T item)
{
    return (*vertices).outgoingedges(item);
}

template<class T> void digraph<T>::prettyprint(void)
{
    cout << "\n\n====== Graph ======\n\n";
...
// For acyclicity checking we need to detect, if a non-tree edge is a backward edge. This is the case, if the finishing time has not yet been set, so the value is -1.
template<class T> int deepsearchtree<T>::getfinish(T node)
{
    deeptreenode<T> * cursor = first;
    while (cursor != last)   // search until we find the node
    {
        if ((*cursor).label() == node)
            return (*cursor).finishtime();
        cursor = (*cursor).getnext();
    }
    if ((*cursor).label() == node)
        return (*cursor).finishtime();
    return -1; // if we assume that node appears in the list, this case will never be reached
}
...
template<class T> searchtree<T> * digraphtraversal<T>::bfs(T startnode)
{
    vertexlist<T> * vertices = digraph<T>::vertices;
...
template<class T> deepsearchtree<T> * digraphtraversal<T>::dfs(T startnode, AList<T> *startlist, bool &nocycle)
{
    vertexlist<T> * vertices = digraph<T>::vertices;
    if ((*vertices).contains(startnode) == false)
    {
    ...
    _dfs(startnode, startnode, vertices, output, markedvertices, dfspos, finishingtime, startlist, nocycle);
    return output;
}

/* auxiliary functions for depth-first search */
template<class T> void digraphtraversal<T>::_dfs(T node1, T node2, vertexlist<T> * vertices, deepsearchtree<T> * output, markerlist<T> * markedvertices, int &dfspos, int &finishingtime, AList<T> *startlist, bool &nocycle)
{
    ...
    if (neighbours != 0)
    {
        ...
        {
            T nextnode = (*cursor).getitem();
            (*startlist).takeoff(nextnode);
            if ((*markedvertices).ismarked(nextnode) == true)
            {
                _traverse_nontreeedge(node2, nextnode, output, nocycle);
                if (!nocycle)
                    return;
            }
            else
            {
                ...
                _dfs(node2, nextnode, vertices, output, markedvertices, dfspos, finishingtime, startlist, nocycle);
                if (!nocycle)
                    return;
            }
            cursor = (*cursor).getnext();
...
template<class T> void digraphtraversal<T>::_traverse_treeedge(T first, T second, deepsearchtree<T> * output, int &dfspos)
{
...
template<class T> void digraphtraversal<T>::_traverse_nontreeedge(T first, T second, deepsearchtree<T> *output, bool &nocycle)
{
    if ((*output).occurs(second))  /* Check if (first,second) is a backward edge, which is the case, if second appears in the output list, but its finishing time is still -1 (i.e. undefined). */
    {
        int fin = (*output).getfinish(second);
        if (fin == -1)   // backward edge detected
            nocycle = false;
    }
    return;
}

template<class T> void digraphtraversal<T>::_backtrack(T node1, T node2, deepsearchtree<T> * output, int &dfspos, int &finishingtime)
{
...
template<class T> bool digraphtraversal<T>::acyclic(void)
{
    bool result = true;
    vertexlist<T> *vertices = digraph<T>::vertices; // get the list of vertices
    if ((*vertices).getlength() == 0)  // the graph is empty
        return result;  // an empty graph is acyclic
    AList<T> *startlist = (*vertices).candidatelist(); /* We determine the list of those vertices that do not have an incoming edge. These vertices are possible start nodes. */
    while (result && (*startlist).getlength() > 0) /* As long as result is true, no cycle has been found, so we continue, if there is still a startnode that needs to be explored. */
    {
        T startnode = (*startlist).getitem(1); // Take the first possible startnode
        (*startlist).remove(1); // eliminate the selected startnode from the list of startnodes that still need to be explored
        deepsearchtree<T> *outtree = dfs(startnode, startlist, result);
        /* Use depth-first search with the selected startnode. Any vertex explored in the search should be eliminated from the startlist. If a cycle is found, result will become false. */
    }
    return result;
}
