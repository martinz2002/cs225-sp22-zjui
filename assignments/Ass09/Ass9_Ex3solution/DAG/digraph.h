...
public:
    ...
    void takeoff(T item);         // added for acyclicity checking
...
public:
    ...
    AList<T> * candidatelist(void);  // added for acyclicity checking
private:
    vertex<T> * dummy;
    int numvertices;
};
...
public:
    ...
    bool occurs(T node);
    void prettyprint(void); // only for testing
    int getfinish(T node);  // added for acyclicity checking
private:
...
template<class T> class digraphtraversal : public digraph<T>
{
public:
    digraphtraversal(void) : digraph<T>(){};
    searchtree<T> * bfs (T startnode);
    deepsearchtree<T> * dfs (T startnode, AList<T> *startlist, bool &nocycle); // modified arguments for acyclicity checking
    bool acyclic (void); // added for acyclicity checking
private:
    void _dfs(T node1, T node2, vertexlist<T> * vertices, deepsearchtree<T> * output, markerlist<T> * markedvertices, int &dfspos, int &finishingtime, AList<T> *startlist, bool &nocycle); // modified arguments for acyclicity checking
    void _traverse_treeedge(T first, T second, deepsearchtree<T> * output, int &dfspos);
        // The second argument will not be used.
    void _traverse_nontreeedge(T first, T second, deepsearchtree<T> *output, bool &nocycle); // modified for acyclicity checking
