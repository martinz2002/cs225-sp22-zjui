...
template<class T> deepsearchtree<T> * digraphtraversal<T>::dfs(T startnode)
{
    vertexlist<T> * vertices = digraph<T>::vertices;
    if ((*vertices).contains(startnode) == false)
    {
...
template<class T> void digraphtraversal<T>::_dfs(T node1, T node2, vertexlist<T> * vertices, deepsearchtree<T> * output, markerlist<T> * markedvertices, int &dfspos, int &finishingtime)
{
...
template<class T> void digraphtraversal<T>::_traverse_nontreeedge(T first, T second)
{
    return;
}
