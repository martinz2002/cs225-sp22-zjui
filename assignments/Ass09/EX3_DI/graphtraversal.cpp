...
template <class T>
void AListsort<T>::qsort(int thr)
{
    int numitems = AList<T>::numitems;
    T *reprarray;
    reprarray = AList<T>::reprarray;
...
template <class T>
DIgraph<T>::DIgraph(void)
{
    vertices = new vertexlist<T>;
}

template <class T>
int DIgraph<T>::numvertices(void)
{
    return (*vertices).getlength();
}

template <class T>
int DIgraph<T>::numedges(void)
{
    return (*vertices).countedges();
}

template <class T>
void DIgraph<T>::addedge(T origin, T destination)
{
...
template <class T>
void DIgraph<T>::addvertex(T item)
{
...
template <class T>
void DIgraph<T>::deleteedge(T origin, T destination)
{
...
template <class T>
void DIgraph<T>::deletevertex(T item)
{
...
template <class T>
edgelist<T> *DIgraph<T>::outgoingedges(T item)
{
    return (*vertices).outgoingedges(item);
}

template <class T>
void DIgraph<T>::prettyprint(void)
{
    cout << "\n\n====== Graph ======\n\n";
...
template <class T>
treenode<T>::treenode(T label, T par, int dist, treenode<T> *pt)
{
    vertexlabel = label;
    parentlabel = par;
    distancetoroot = dist;
...
template <class T>
T treenode<T>::label(void)
{
    return vertexlabel;
}

template <class T>
T treenode<T>::parent(void)
{
    return parentlabel;
}

template <class T>
int treenode<T>::distance(void)
{
    return distancetoroot;
}

template <class T>
treenode<T> *treenode<T>::getnext(void)
{
    return next;
}

template <class T>
void treenode<T>::setlabel(T item)
{
    vertexlabel = item;
    return;
}
...
template <class T>
void treenode<T>::setparent(T item)
{
    parentlabel = item;
    return;
}
...
template <class T>
void treenode<T>::setdistance(int dist)
{
    distancetoroot = dist;
    return;
}
...
template <class T>
void treenode<T>::setnext(treenode<T> *pt)
{
    next = pt;
    return;
}
...
/* only for testing */
template <class T>
void treenode<T>::prettyprint(void)
{
    if (parent() != label())
...
template <class T>
void searchtree<T>::add(T label, T parentlabel, int dist)
{
...
template <class T>
bool searchtree<T>::hasparent(T node)
{
    bool result = false;
    if (first == 0)
        return result;
...
template <class T>
void searchtree<T>::prettyprint(void)
{
    if (first == 0)
    {
        cout << "The DIgraph is empty.\n";
        return;
    }
    treenode<T> *cursor = first;
    while (cursor != last)
...
template <class T>
deeptreenode<T>::deeptreenode(T node, int pos, int finish, deeptreenode<T> *pt)
{
    nodelabel = node;
    dfspos = pos;
...
template <class T>
void markerlist<T>::unmark(T item)
{
    if (first == 0)
        return;
    if (first == last)
...
template <class T>
searchtree<T> *DI_graphtraversal<T>::bfs(T startnode)
{
...
template <class T>
deepsearchtree<T> *DI_graphtraversal<T>::dfs(T startnode)
{
    vertexlist<T> *vertices = DIgraph<T>::vertices;
    if ((*vertices).contains(startnode) == false)
    {
...
template <class T>
void DI_graphtraversal<T>::_dfs(T node1, T node2, vertexlist<T> *vertices, deepsearchtree<T> *output, markerlist<T> *markedvertices, int &dfspos, int &finishingtime)
{
...
template <class T>
void DI_graphtraversal<T>::_traverse_treeedge(T first, T second, deepsearchtree<T> *output, int &dfspos)
{
...
template <class T>
void DI_graphtraversal<T>::_traverse_nontreeedge(T first, T second)
{
    return;
}

template <class T>
void DI_graphtraversal<T>::_backtrack(T node1, T node2, deepsearchtree<T> *output, int &dfspos, int &finishingtime)
{
...
template <class T>
bool DI_graphtraversal<T>::is_acyclic(T startnode)
{
    vertexlist<T> *vertices = DIgraph<T>::vertices;
    if ((*vertices).contains(startnode) == false)
    {
