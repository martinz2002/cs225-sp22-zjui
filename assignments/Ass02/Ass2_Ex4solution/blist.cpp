...
{
    T * reprarray = AList<T>::reprarray;
    int n = AList<T>::numitems;
    int min = AList<T>::getmin();
    int max = AList<T>::getmax();
    // initialise buckets
    AList<T> * buckets = new AList<T>[n]();
    for (int i = 0; i < n; i++)
    {
        (buckets[i]).setmax(max);
        (buckets[i]).setmin(min);
    }
    T * knots = new T[n+1];
    for (int j = 0; j <= n; j++)
    {
        knots[j] = min + (j * (max - min) / n);
    }
    // insertion sort into the buckets
    int len, last;
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            if (knots[j] <= reprarray[i] && reprarray[i] < knots[j+1])
            {
                if (buckets[j].getlength() == 0)
                {
                    buckets[j].append(reprarray[i]);
                    break;
                }
                len = buckets[j].getlength();
                last = (buckets[j])[len];
                if (reprarray[i] > last)
                {
                    buckets[j].append(reprarray[i]);
                ...
                for (int k = 1; k <= (buckets[j]).getlength(); k++)
                {
                    if (reprarray[i] <= (buckets[j])[k])
                    {
                        buckets[j].insert(k, reprarray[i]);
                        break;
                    }
                }
            }
