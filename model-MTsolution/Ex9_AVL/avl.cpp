...
{
    AList<T> *orderedlist = enumerate();
    int length = (*orderedlist).getlength();
    int med = (length + 1) / 2;
    return (*orderedlist).getitem(med);
}

template<class T> AList<T> *AVL<T>::enumerate(void)
{
    AList<T> *result = _enumerate(root);
    return result;
}

template<class T> AList<T> *AVL<T>::_enumerate(avlnode<T> *pt)
{
    AList<T> *result = new AList<T>();
    if (pt != 0)
    {
        AList<T> *res1 = _enumerate((*pt).getleft());
        (*result).concat(*res1);
        (*result).append((*pt).getdata());
        AList<T> *res2 = _enumerate((*pt).getright());
        (*result).concat(*res2);
    }
    return result;
}
