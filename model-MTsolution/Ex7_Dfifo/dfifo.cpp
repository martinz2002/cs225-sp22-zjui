...
/*
template<class T> T Dfifo<T>::operator[](int index)
{
    T val = getitem(index);
    return val;
} */
...
{
    // We obtain the reversed list, if for all nodes the next and previous pointers are swapped.
    node<T> *pt;
    node<T> *ptn;
    pt = (*dummy).getnext();
    // First swap the pointers for the dummy node
    (*dummy).setnext((*dummy).getprevious());
    (*dummy).setprevious(pt);
    // Next swap the pointers for all other nodes
    for (int i = 0; i < numitems; i++)
    {
        ptn = (*pt).getnext();
        (*pt).setnext((*pt).getprevious());
        (*pt).setprevious(ptn);
        pt = ptn;
    }
    return;
