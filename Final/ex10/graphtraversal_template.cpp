...
template<class T> void elist<T>::pushfront(T item)
{
    neighbour<T> * pt_last = (*first).getprevious();
    neighbour<T> * pt_new = new neighbour<T>;
    (*pt_new).setitem(item);
...
template<class T> void elist<T>::popback(void)
{
    if (length <= 1)
        return;
    neighbour<T> * pt_last = (*first).getprevious();
...
template<class T> dirgraph<T>::dirgraph(void)
{
    vertices = new vertexlist<T>;
}

template<class T> int dirgraph<T>::numvertices(void)
{
    return (*vertices).getlength();
}

template<class T> int dirgraph<T>::numedges(void)
{
    return (*vertices).countedges();
}

template<class T> void dirgraph<T>::addedge(T origin, T destination)
{
    (*vertices).addedge(origin, destination);
    return;
}

template<class T> void dirgraph<T>::addvertex(T item)
{
    (*vertices).addvertex(item);
    return;
}

template<class T> void dirgraph<T>::deleteedge(T origin, T destination)
{
    (*vertices).removeedge(origin, destination);
    return;
}

template<class T> void dirgraph<T>::deletevertex(T item)
{
    (*vertices).remove(item);
    return;
}

template<class T> void dirgraph<T>::prettyprint(void)
{
    cout << "\n\n====== Graph ======\n\n";
...
template<class T> elist<T> * graphtraversal<T>::topol_order(T startnode)
{
    /* This member function needs to be implemented by modifying dfs */
}

/* auxiliary functions for depth-first search */
template<class T> void graphtraversal<T>::_dfs(T node, vertexlist<T> * vertices, elist<T> * output, markerlist<T> * markedvertices)
{
...
template<class T> void graphtraversal<T>::_traverse_nontreeedge(T node, elist<T> *output)
{
    /* The traversal of non-tree edges needs to check for cycles */
}

template<class T> bool graphtraversal<T>::suitablestart(T item)
{
